    ...
    S: Strategy,
{
    ...
    #[inline]
    #[must_use]
    pub fn with_strategy(v: T, dropfn: F) -> Scope<T, F, S> {
        Scope {
    ...
    ///         let value = Scope::into_inner(guard);
    ///     } else {
...
#[inline]
#[must_use]
pub fn guard_on_success<T, F>(v: T, dropfn: F) -> Scope<T, F, OnSuccess>
where
    F: FnOnce(T),
{
    Scope::with_strategy(v, dropfn)
...
#[inline]
#[must_use]
pub fn guard_on_unwind<T, F>(v: T, dropfn: F) -> Scope<T, F, OnUnwind>
where
    F: FnOnce(T),
{
    Scope::with_strategy(v, dropfn)
...
unsafe impl<T, F, S> Sync for Scope<T, F, S>
where
    T: Sync,
    F: FnOnce(T),
    S: Strategy,
...
impl<T, F, S> Deref for Scope<T, F, S>
where
    F: FnOnce(T),
    S: Strategy,
{
...
impl<T, F, S> DerefMut for Scope<T, F, S>
where
    F: FnOnce(T),
    S: Strategy,
{
...
impl<T, F, S> Drop for Scope<T, F, S>
where
    F: FnOnce(T),
    S: Strategy,
{
...
impl<T, F, S> fmt::Debug for Scope<T, F, S>
where
    T: fmt::Debug,
    F: FnOnce(T),
    S: Strategy,
